...
def answer_question(csv_path: str, question: str, audit_dir: str | None = None) -> dict[str, Any]:
    ...
    if intent_type not in _INTENT_TO_COLUMN_KEYS:
        result = ask_question(csv_path, question, audit_dir=str(audit_path))
        result["tier"] = "B"
    ...
    if "error" in result:
        tier_b_result = ask_question(csv_path, question, audit_dir=str(audit_path))
